...
export function SegmentsTable({
    ...
    const expectMouseGeneratedSegmentFocus = () => {
        skipNextFocusedSegmentSelection.current = true;
        // Focus caused by this mouse-down is delivered before the next event
        // loop task. Expire the token afterwards so clicking an input that was
        // already focused cannot leave it behind for a later Tab / Shift+Tab
        // navigation and make that newly focused row edit the wrong segment.
        window.setTimeout(() => {
    ...
    const handleSegmentInputMouseDown = (
        ...
        if (focusClickedRow) {
            expectMouseGeneratedSegmentFocus();
        } else {
            skipNextFocusedSegmentSelection.current = false;
        }
                        ...
                        return (
                            <tr
                                ...
                                onMouseDown={(e) => {
                                    ...
                                    e.preventDefault();
                                    // A pending token only describes the focus
                                    // generated by a segment input's own mouse
                                    // down. A group-header interaction starts a
                                    // new selection operation, so it must not
                                    // affect whichever segment receives focus
                                    // later through keyboard navigation.
                                    skipNextFocusedSegmentSelection.current = false;
                                    selectSegmentGroup(
                                        editor,
                                        groupIndex,
                            ...
                            >
                                ...
                                <td colSpan={columnCount - 1}>
                                    <input
                                        ...
                                        onFocus={(_) => {
                                            // Focusing the editable header is
                                            // not the follow-up focus event of
                                            // a segment mouse-down. Expire any
                                            // unconsumed token before selecting
                                            // the group so Tab cannot skip the
                                            // next segment's selection.
                                            skipNextFocusedSegmentSelection.current = false;
                                            selectSegmentGroup(
                                                editor,
                                                groupIndex,
